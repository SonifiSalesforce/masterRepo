...
// SEQNO__c custom -- from dialer record
	...
	try {
		...
		queryParams.add("Enghouse_CCSP__Phone__c", phone_c);
		queryParams.add("Enghouse_CCSP__DNIS__c", dnis_c);
		...
		queryParams.add("Enghouse_CCSP__CallType__c", callType_c);
		...
		queryParams.add("Enghouse_CCSP__CallStartTime__c", getDateTime(callStartTime_c))
	
		// CallEndTime__c custom -- call end time
		var callEndTime_c = callData.CtcCallEndTime || now.toISOString();
		queryParams.add("Enghouse_CCSP__CallEndTime__c", getDateTime(callEndTime_c));
		...
		queryParams.add("Enghouse_CCSP__WrapupComment__c", comment);

		// SEQNO__c custom -- from dialer record
		var seqno_c = "";
		if (dialerRecord) {
			seqno_c = callDataHelper.getDialerDataByName(dialerRecord, "SEQNO");
			queryParams.add("Enghouse_CCSP__SEQNO__c", seqno_c, true);
		}

		// ChatTranscript__c
		if (userData && userData.length > 0) {
			queryParams.add("Enghouse_CCSP__ChatTranscript__c", getChatTranscript(userData), true);
		}
